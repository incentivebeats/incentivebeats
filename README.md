### James B Osborne / Incentive / Chuudoku

*Electronic Music Artist, Linux Administrator, Infra Designer, DevOps Engineer, EFF Member*  
  
I make music and like Linux & FreeBSD too much.

My music, technical work, and social links  
🌐 [circle with a dot](https://circlewithadot.net)  

My own git, where I have more repos available  
🌐 [cwad@gitea](https://git.circlewithadot.net/CWAD)  
🌐 [incentive@gitea](https://git.circlewithadot.net/incentive)  

📧 E-mail: `incentive@circlewithadot.net`  
💬 Matrix: `@incentive:circlewithadot.net`

---
#### Phantasy Star Online

- [psopeeps-newserv](https://github.com/incentivebeats/psopeeps-newserv)  
  Fork/custom server work

- [psopeeps-ansible](https://github.com/incentivebeats/psopeeps-ansible)  
  Ansible deployment/configuration work for newserv

- [psopeeps-newserv-timers](https://github.com/incentivebeats/psopeeps-newserv-timers)  
  Systemd timer/service helpers

- [pso_pc-dll_files](https://github.com/incentivebeats/pso_pc-dll_files)  
  DLL/release assets for PSO PC work.

- [pso_pc-dragon_fix](https://github.com/incentivebeats/pso_pc-dragon_fix)  
  PSO PC Dragon asset/fix research and patch work.

#### Other

- [circlewithadot.net](https://git.circlewithadot.net/CWAD/circlewithadot.net)  
  Source for my personal/site infrastructure.

- [relibre](https://git.circlewithadot.net/incentive/relibre)  
  A simple, free and open source landing page generator for self-hosted music release pages.
